...
router.get("/:eventId/registrations/excel", async (req, res) => {
  ...
  try {
    ...
    const maxEvents = Math.max(
      ...userData.map((user) => (user.orders?.[0]?.events || []).length)
    );
    ...
    const sheetDataPromises = userData.map(async (user) => {
      ...
      const rowData = {
        ...
        "Transaction ID":
          user.orders?.[0]?.paymentDetails?.transactionId || "N/A",
        ...
        "Registration Date": new Date(
          user.orders?.[0]?.paymentDetails?.date
        ).toLocaleString(),
        "Total Events Registered": eventNames.length,
      };

      // Add each event to a separate column
      for (let i = 0; i < maxEvents; i++) {
        rowData[`Event ${i + 1}`] = eventNames[i] || "-";
      }
...
router.put("/:eventId", async (req, res) => {
  ...
  try {
    ...
    event.type = type;
    (event.isHidden = isHidden), (event.duration = duration);
